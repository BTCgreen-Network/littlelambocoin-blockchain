...
# Gets the version from the result of "littlelambocoin version"
...
def get_littlelambocoin_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["littlelambocoin", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_littlelambocoin_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/littlelambocoin-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/littlelambocoin-blockchain-gui/packages/gui/package.json")
