    ...
    async def new_transaction(
        ...
        if self.full_node.mempool_manager.is_fee_enough(transaction.fees, transaction.cost):
            ...
            async def tx_request_and_timeout(full_node: FullNode, transaction_id, task_id):
                ...
                try:
                    while True:
                        # Limit to asking to 10 peers, it's possible that this tx got included on chain already
                        # Highly unlikely that the 10 peers that advertised a tx don't respond to a request. Also, if we
