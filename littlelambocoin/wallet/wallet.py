...
class Wallet:
    ...
    async def create_spend_bundle_relative_littlelambocoin(self, littlelambocoin_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if littlelambocoin_amount < 0:
            utxos = await self.select_coins(uint64(abs(littlelambocoin_amount)), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(littlelambocoin_amount), "memos": []}
                ]
    ...
    # WSLittlelambocoinConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSLittlelambocoinConnection
