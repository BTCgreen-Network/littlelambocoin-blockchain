...
from littlelambocoin.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_tandem_llc_tx(
        ...
        if fee > amount_to_claim:
            littlelambocoin_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
                ...
                if need_littlelambocoin_transaction:
                    if fee > regular_littlelambocoin_to_claim:
                        littlelambocoin_tx, _ = await self.create_tandem_llc_tx(
                            fee,
                            uint64(regular_littlelambocoin_to_claim),
                        ...
                        littlelambocoin_tx, _ = await self.create_tandem_llc_tx(
                            fee, uint64(regular_littlelambocoin_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
