...
from littlelambocoin.wallet.nft_wallet import nft_puzzles
from littlelambocoin.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from littlelambocoin.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from littlelambocoin.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from littlelambocoin.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
...
from littlelambocoin.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
from littlelambocoin.wallet.transaction_record import TransactionRecord
from littlelambocoin.wallet.uncurried_puzzle import uncurry_puzzle
from littlelambocoin.wallet.util.compute_memos import compute_memos
from littlelambocoin.wallet.util.debug_spend_bundle import disassemble
from littlelambocoin.wallet.util.transaction_type import TransactionType
from littlelambocoin.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
from littlelambocoin.wallet.wallet_nft_store import WalletNftStore
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSLittlelambocoinConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSLittlelambocoinConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        if littlelambocoin_tx is not None and littlelambocoin_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, littlelambocoin_tx.spend_bundle])
            littlelambocoin_tx = dataclasses.replace(littlelambocoin_tx, spend_bundle=None)
        ...
        if littlelambocoin_tx is not None:
            tx_list.append(littlelambocoin_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            littlelambocoin_tx = await self.create_tandem_llc_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, littlelambocoin_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'LLC'} to offer")
    ...
    async def mint_from_did(
        ...
        llc_coins: Optional[Set[Coin]] = None,
        llc_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an llc spend that funds the transactions and pays fees.
        ...
        :param llc_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param llc_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the llc spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an llc coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if llc_coins is None:
            llc_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(llc_coins) > 0
        ...
        # Create the llc spend to fund the minting.
        spend_value = sum([coin.amount for coin in llc_coins])
        change: uint64 = uint64(spend_value - total_amount)
        llc_spends = []
        if llc_change_ph is None:
            llc_change_ph = await self.standard_wallet.get_new_puzzlehash()
        llc_primaries = [
            AmountWithPuzzlehash({"puzzlehash": llc_change_ph, "amount": change, "memos": [llc_change_ph]})
        ]

        first = True
        for llc_coin in llc_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(llc_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in llc_coins]
                message_list.append(
                    Coin(llc_coin.name(), llc_primaries[0]["puzzlehash"], llc_primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(llc_coins) > 1:
                    llc_announcement: Optional[Set[bytes]] = {message}
                else:
                    llc_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=llc_primaries,
                    fee=fee,
                    coin_announcements=llc_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(llc_coin.name(), message).name()
            ...
            llc_spends.append(CoinSpend(llc_coin, puzzle, solution))
        llc_spend = await self.standard_wallet.sign_transaction(llc_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, llc_spend, *eve_spends])
        return total_spend

    async def mint_from_llc(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        """
        Minting NFTs from a single LLC spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = llc_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from LLC to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the LLC spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the LLC spend
            ...
            # Annnouncements for eve spend. These are asserted by the llc spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for llc_coin in llc_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=llc_primaries + primaries,
                    fee=fee,
                    coin_announcements=llc_announcement if len(llc_coins) > 1 else None,
