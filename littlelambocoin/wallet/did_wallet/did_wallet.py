...
from littlelambocoin.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
from littlelambocoin.wallet.coin_selection import select_coins
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSLittlelambocoinConnection):
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        peer: WSLittlelambocoinConnection = wallet_node.get_full_node_peer()
        if peer is None:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if littlelambocoin_tx is not None and littlelambocoin_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(littlelambocoin_tx)
