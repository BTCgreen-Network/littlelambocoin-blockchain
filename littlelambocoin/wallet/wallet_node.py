...
from littlelambocoin.util.chunks import chunks
from littlelambocoin.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from littlelambocoin.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from littlelambocoin.util.profiler import mem_profile_task, profile_task
from littlelambocoin.wallet.transaction_record import TransactionRecord
from littlelambocoin.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from littlelambocoin.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from littlelambocoin.wallet.util.wallet_sync_utils import (
...
from littlelambocoin.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    _server: Optional[LittlelambocoinServer] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.littlelambocoin.com"]),
    ...
    def on_disconnect(self, peer: WSLittlelambocoinConnection):
    ...
    async def long_sync(
        ...
        full_node: WSLittlelambocoinConnection,
        fork_height: int,
        *,
        rollback: bool,
    ):
    ...
    async def is_peer_synced(
        self, peer: WSLittlelambocoinConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSLittlelambocoinConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSLittlelambocoinConnection]:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSLittlelambocoinConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            return None
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSLittlelambocoinConnection]:
        """
        ...
        synced_and_trusted: List[WSLittlelambocoinConnection] = []
        synced: List[WSLittlelambocoinConnection] = []
        trusted: List[WSLittlelambocoinConnection] = []
        neither: List[WSLittlelambocoinConnection] = []
        all_nodes: List[WSLittlelambocoinConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peers: List[WSLittlelambocoinConnection] = self.get_full_node_peers_in_order()
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSLittlelambocoinConnection):
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSLittlelambocoinConnection):
    ...
    async def new_peak_from_untrusted(
        self, new_peak_hb: HeaderBlock, peer: WSLittlelambocoinConnection, request_time: uint64
    ) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSLittlelambocoinConnection):
        current_height: uint32 = await self.wallet_state_manager.blockchain.get_finished_sync_up_to()
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSLittlelambocoinConnection) -> int:
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSLittlelambocoinConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSLittlelambocoinConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def fetch_puzzle_solution(self, height: uint32, coin: Coin, peer: WSLittlelambocoinConnection) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSLittlelambocoinConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSLittlelambocoinConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
