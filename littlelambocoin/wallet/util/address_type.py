...
from littlelambocoin.util.config import selected_network_address_prefix


class AddressType(Enum):
    LLC = "llc"
    NFT = "nft"
    DID = "did:littlelambocoin:"

    def hrp(self, config: Dict[str, Any]) -> str:
        if self == AddressType.LLC:
            # Special case to map LLC to the current network's address prefix
