    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["pool"].get("llc_target_address") or all_targets[-2] == config["pool"].get(
                    "llc_target_address"
                ):
        ...
        if "llc_target_address" not in config["farmer"]:
            print(
                f"Setting the llc destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["llc_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["llc_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['llc_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "llc_target_address" not in config["pool"]:
            print(f"Setting the llc destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["llc_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["llc_target_address"] not in all_targets:
            print(
...
def init(
    ...
    if create_certs is not None:
        ...
        return littlelambocoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def littlelambocoin_init(
    ...
    *,
    ...
    """
    littlelambocoin_root = os.environ.get("LITTLELAMBOCOIN_ROOT", None)
    if littlelambocoin_root is not None:
        print(f"LITTLELAMBOCOIN_ROOT is set to {littlelambocoin_root}")
