...
from littlelambocoin.wallet.derive_chives_keys import chives_master_sk_to_farmer_sk, chives_master_sk_to_pool_sk
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'littlelambocoin keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from littlelambocoin.wallet.derive_keys import _derive_path, _derive_path_unhardened
...
async def migrate_keys(root_path: Path, forced: bool = False) -> bool:
    ...
    deprecation_message = (
        ...
        "You need to migrate your keyring to continue using Littlelambocoin.\n"
    )
    ...
    if Keychain.needs_migration():
        ...
        if len(keys_to_migrate) > 0:
            ...
            if not prompt_yes_no("Migrate these keys?"):
                ...
                print("Migration aborted, can't run any littlelambocoin commands.")
                return False
