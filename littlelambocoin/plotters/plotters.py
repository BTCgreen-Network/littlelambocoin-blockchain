...
littlelambocoin_plotter_options = [
    Options.TMP_DIR,
    Options.TMP_DIR2,
    Options.K,
    Options.MEMO,
...
def call_plotters(root_path: Path, args):
    ...
    plotters = argparse.ArgumentParser("littlelambocoin plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, littlelambocoin_plotter_options, "littlelambocoinpos", "Create a plot with the default littlelambocoin plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'littlelambocoin plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    if args.plotter is None:
        plotters.print_help()
    elif args.plotter == "littlelambocoinpos":
        ...
        show_plotters_version(littlelambocoin_root_path)
...
def show_plotters_version(root_path: Path):
    ...
    if "littlelambocoinpos" in info and "version" in info["littlelambocoinpos"]:
        print(f"littlelambocoinpos: {info['littlelambocoinpos']['version']}")
