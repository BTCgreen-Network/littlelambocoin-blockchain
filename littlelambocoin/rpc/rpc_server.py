...
from littlelambocoin.rpc.util import wrap_http_handler
from littlelambocoin.server.outbound_message import NodeType
from littlelambocoin.server.server import LittlelambocoinServer, ssl_context_for_client, ssl_context_for_server
...
from littlelambocoin.util.config import str2bool
...
from littlelambocoin.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> LittlelambocoinServer:
        ...
        # a property so as to be read only which allows LittlelambocoinServer to satisfy
        # Optional[LittlelambocoinServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        littlelambocoin.rpc.rpc_server.default_get_connections()
        """
        ...

    async def on_connect(self, peer: WSLittlelambocoinConnection) -> None:
...
def default_get_connections(server: LittlelambocoinServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
