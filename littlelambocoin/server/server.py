...
from littlelambocoin.protocols.protocol_state_machine import message_requires_reply
from littlelambocoin.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
...
from littlelambocoin.util.api_decorators import get_metadata
from littlelambocoin.util.errors import Err, ProtocolError
from littlelambocoin.util.ints import uint16
from littlelambocoin.util.network import WebServer, is_in_network, is_localhost
from littlelambocoin.util.ssl_check import verify_ssl_certs_and_keys
...
class LittlelambocoinServer:
    ...
    all_connections: Dict[bytes32, WSLittlelambocoinConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        littlelambocoin_ca_crt_path, littlelambocoin_ca_key_path = littlelambocoin_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                littlelambocoin_ca_crt_path, littlelambocoin_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                littlelambocoin_ca_crt_path, littlelambocoin_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    async def start_server(self, prefer_ipv6: bool, on_connect: Callable = None):
        if self.webserver is not None:
            raise RuntimeError("LittlelambocoinServer already started")
    ...
    async def incoming_connection(self, request):
        ...
        try:
            connection = WSLittlelambocoinConnection.create(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    def connection_closed(self, connection: WSLittlelambocoinConnection, ban_time: int, closed_connection: bool = False):
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use littlelambocoin's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.littlelambocoin.com/") as resp:
                    if resp.status == 200:
