...
## 1.4.0 Littlelambocoin blockchain 2022-6-29

### Added

- Added support for NFTs!!! :party:
- Added `littlelambocoin wallet nft` command (see https://docs.littlelambocoin.com/docs/13cli/did_cli)
- Added `littlelambocoin wallet did` command (see https://docs.littlelambocoin.com/docs/12rpcs/nft_rpcs)
- Added RPCs for DID (see https://docs.littlelambocoin.com/docs/12rpcs/did_rpcs)
- Added RPCs for NFT (see https://docs.littlelambocoin.com/docs/12rpcs/nft_rpcs)
...
- Added CLI options `--sort-by-height` and –sort-by-relevance` to `littlelambocoin wallet get_transactions`
...
- New coin selection algorithm based on bitcoin knapsack. Previously littlelambocoin selected the largest coin
...
- Updated warning about `LITTLELAMBOCOIN_ROOT` being set when running init
...
## 1.3.5 Littlelambocoin blockchain 2022-5-11
...
## 1.3.4 Littlelambocoin blockchain 2022-4-19
...
- Implemented `littlelambocoin rpc` command
...
- Added `-n`/`--new-address` option to `littlelambocoin wallet get_address`
...
- Fixed typo in command line argument parsing for `littlelambocoin db validate`
- Improved backwards compatibility for node RPC calls `get_blockchain_state` and `get_additions_and_removals`
- Fixed issue where `--root_path` option was not honored by `littlelambocoin configure` CLI command
- Fixed cases where node DB was not created initially using v2 format
- Improved error messages from `littlelambocoin db upgrade`
- Capitalized display of `Rpc` -> `RPC` in `littlelambocoin show -s` by (Thanks @hugepants)
...
- Minor output formatting/enhancements for `littlelambocoin wallet show`
- Fixed typo and index issues in wallet database
- Used the rust clvm version instead of python in more places
- Fixed trailing bytes shown in CAT asset ID row when using `littlelambocoin wallet show`
...
- You cannot install and run littlelambocoin blockchain using the macOS packaged DMG on macOS Mojave (10.14).
...
## 1.3.3 Littlelambocoin blockchain 2022-4-02
...
## 1.3.2 Littlelambocoin blockchain 2022-4-01

### Fixed

- Fixed OpenSSL vulnerability CVE-2022-0778

## 1.3.1 Littlelambocoin blockchain 2022-3-16

### Fixed

- Improved config.yaml update concurrency to prevent some cases of the wrong pool being used for a PlotNFT.
- Fixed `littlelambocoin keys show` displaying non-observer-derived wallet address.
...
- Fixed exception when `littlelambocoin keys migrate` is run without needing migration.
...
- Added checks to ensure wallet address prefixes are either `llc` or `tllc`.
...
## 1.3.0 Littlelambocoin blockchain 2022-3-07
...
- Added new RPC, get_version, to the daemon to return the version of Littlelambocoin (Thanks @dkackman).
- Added new config.yaml setting, reserved_cores, to specify how many cores Littlelambocoin will not use when launching process pools. Using 0 will allow Littlelambocoin to use all cores for process pools. Set the default to 0 to allow Littlelambocoin to use all cores. This can result in faster syncing and better performance overall especially on lower-end CPUs like the Raspberry Pi4.
- Added new RPC, get_logged_in_fingerprint, to the wallet to return the currently logged in fingerprint.
- Added new CLI option, littlelambocoin keys derive, to allow deriving any number of keys in various ways. This is particularly useful to do an exhaustive search for a given address using littlelambocoin keys derive search.
...
- CLI option littlelambocoin show will display the currently selected network (mainnet or testnet).
- CLI option littlelambocoin plots check will display the Pool Contract Address for Portable (PlotNFT) plots.
...
- Fixed issues with displaying IPv6 address in CLI with littlelambocoin show -c.
- Thanks to @chuwt for fix to looping logic during node synching.
- Fixed the littlelambocoin-blockchain RPM to set the permission of chrome-sandbox properly.
...
- Fixed display info in CLI littlelambocoin show -c when No Info should be displayed.
...
- When you close the Littlelambocoin app, regardless if you are in farmer mode or wallet, the content on the exit dialog isn’t correct.
- If you start with wallet mode and then switch to farmer mode and back to wallet mode, the full node will continue to sync in the background. To get the full node to stop syncing after switching to wallet mode, you will need to close the Littlelambocoin and relaunch the Littlelambocoin app.
...
Farmers rejoice: today's release integrates two plotters in broad use in the Littlelambocoin community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/pull/9050).
...
We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Passphrase-Protected-Littlelambocoin-Keys-and-Key-Storage-Migration). We also launched a new Littlelambocoinlisp compiler in clvm_tools_rs which substantially improves compile time for Littlelambocoinlisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Passphrase-Protected-Littlelambocoin-Keys-and-Key-Storage-Migration).
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other littlelambocoin services.
...
- Updated the old comment on littlelambocoin/util/streamable.py with newer developer documentation.
...
- Beta builds are built on every merge to main, and are now available from <https://littlelambocoin.com/download/>.
...
- Added a py.typed file that allows other projects that pip install littlelambocoin-blockchain to type check using our functions with mypy.
...
- Switched IP address lookup to first use Littlelambocoin's service ip.littlelambocoin.com.
...
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.
...
- Correctly display private key in `littlelambocoin keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `littlelambocoin keys show`.
...
- You should review the [release notes for v1.0.2](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- We have released version 1.0.0 of [chiapos](https://github.com/Littlelambocoin-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- This is a hard fork/breaking change from RC6/7. TCAC Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.littlelambocoin.com/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `littlelambocoin farm summary` will now display TCAC or CAC as appropriate.
...
- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/issues/1189)
...
- We have moved to a new and much higher scalability download.littlelambocoin.com to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.littlelambocoin.com/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.littlelambocoin.com/latest/Setup-MacOS.dmg).
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.littlelambocoin.com/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.littlelambocoin.com via https, for example.
...
- RC3 is a new chain to support the last major littlelambocoinlisp changes. TCAC from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `littlelambocoin -h` and `littlelambocoin SUBCOMMAND -h` can be your guide.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/releases/tag/1.0rc1) was well.
...
- There is now remote UI support. [Documents](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- 'littlelambocoin wallet send' now takes in TCAC or CAC as units instead of mojos.
...
- The GUI is now in a separate submodule repository from littlelambocoin-blockchain, [littlelambocoin-blockchain-gui](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `littlelambocoin-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `littlelambocoin wallet`, was incorrectly displaying TCAC balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [littlelambocoin.com](https://littlelambocoin.com/). Among the improvements this gives the Littlelambocoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Littlelambocoin-Network/chiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Littlelambocoin dependencies like chiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/tree/dev/electron-react/src/locales) of the littlelambocoin-blockchain repository.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Littlelambocoin-Network/chiapos/releases/tag/0.12.32).
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Littlelambocoin Network at backup.littlelambocoin.com. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Littlelambocoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.littlelambocoin.com, your own installation, or a third party's version of it.
...
- The user interface now refers to littlelambocoin coins with their correct currency code of CAC.
...
- There was a regression in Littlelambocoin Proof of Space ([chiapos](https://github.com/Littlelambocoin-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki) and placed in [INSTALL](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Quick-Start-Guide)
...
- We suggest you take a look at our [Upgrading documentation](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- There is a new command `littlelambocoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `littlelambocoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/wiki/Updating-beta-software). If you've set `$LITTLELAMBOCOIN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We have revamped the littlelambocoin management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `littlelambocoin-start-farmer &`. The [README.md](https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Implemented an RPC interface with JSON serialization for streamables - currently on port 18714.
...
- More details on the release at [https://www.littlelambocoin.com/developer/](https://www.littlelambocoin.com/developer/)

[unreleased]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Littlelambocoin-Network/littlelambocoin-blockchain/releases/tag/Alpha-1.0
