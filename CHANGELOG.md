...
- Add condition code constant for REMARK, an always true Littlelambocoinlisp condition
- Add several wallet optimizations
- Add `littlelambocoin db backup --backup-file <backup_file_destination>` (Thanks @neurosis69!)
...
- Documented deserialization length limitations (8191 bytes) in CLVM ROM. We recommend using a local version of the littlelambocoinlisp code when necessary
...
- Remove littlelambocoin.util.path.mkdir()
...
- Removing `logging.basicConfig()` from `LittlelambocoinServer.__init__()`
...
- Log `littlelambocoin_full_version_str` in daemon and all services
...
- Substantially reduced overall size of Littlelambocoin packages
...
- Make sure littlelambocoin commands output help when no args are given (#11013) (Thanks @noneus!)
...
## 1.5.0 Littlelambocoin blockchain 2022-7-26
...
- Update `littlelambocoin wallet take_offer` to show NFT royalties that will be paid out when an offer is taken
- Added a parameter to indicate how many additional puzzle hashes `create_more_puzzle_hashes` should create

### Fixed

- Fixed [CVE-2022-36447] where in tokens previously minted on the Littlelambocoin blockchain using the `CAT1` standard can be inflated in arbitrary amounts by any holder of the token. Total amount of the token can be increased as high as the malicious actor pleases. This is true for every `CAT1` on the Littlelambocoin blockchain, regardless of issuance rules. This attack is auditable on-chain, so maliciously altered coins can potentially be "marked" by off-chain observers as malicious.
...
- Farmers rejoice: today's release integrates two plotters in broad use in the Littlelambocoin community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/pull/9050).
...
We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Passphrase-Protected-Littlelambocoin-Keys-and-Key-Storage-Migration). We also launched a new Littlelambocoinlisp compiler in clvm_tools_rs which substantially improves compile time for Littlelambocoinlisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Passphrase-Protected-Littlelambocoin-Keys-and-Key-Storage-Migration).
...
- Thanks to @yshklarov for help fixing [#7273](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.
...
- You should review the [release notes for v1.0.2](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- Mainnet uses port 4575 and other constants and service names were changed for mainnet.
...
- This is a hard fork/breaking change from RC6/7. TLLC Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.littlelambocoin.com/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `littlelambocoin farm summary` will now display TLLC or LLC as appropriate.
...
- The Farm page will now no longer get stuck at 50 TLLC farmed.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `llc_target_address` sections of config.yaml.
...
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `littlelambocoin -h` and `littlelambocoin SUBCOMMAND -h` can be your guide.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/releases/tag/1.0rc1) was well.
...
- There is now remote UI support. [Documents](https://github.com/BTCgreen-Network/littlelambocoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- 'littlelambocoin wallet send' now takes in TLLC or LLC as units instead of mojos.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TLLC/LLC per block for a total of 9216 TLLC per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from littlelambocoin-blockchain, [littlelambocoin-blockchain-gui](https://github.com/BTCgreen-Network/littlelambocoin-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `littlelambocoin-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `littlelambocoin wallet`, was incorrectly displaying TLLC balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
...
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/tree/dev/electron-react/src/locales) of the littlelambocoin-blockchain repository.
...
- Changed LLC units to TLLC units for testnet.
...
- The user interface now refers to littlelambocoin coins with their correct currency code of LLC.
...
- There was a regression in Littlelambocoin Proof of Space ([chiapos](https://github.com/BTCgreen-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki) and placed in [INSTALL](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Quick-Start-Guide)
...
- We suggest you take a look at our [Upgrading documentation](https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- There is now full transaction support on the Littlelambocoin blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/BTCgreen-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
[unreleased]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/BTCgreen-Network/littlelambocoin-blockchain/releases/tag/Alpha-1.0
