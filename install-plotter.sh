...
get_madmax_filename() {
  ...
  echo "${LITTLELAMBOCOIN_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" littlelambocoin_plot
  chmod 755 littlelambocoin_plot
  echo "Successfully installed madmax to $(pwd)/littlelambocoin_plot"
  ...
  mv -f "${madmax_filename}" littlelambocoin_plot_k34
  chmod 755 littlelambocoin_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/littlelambocoin_plot_k34"
else
  usage
fi
