...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the littlelambocoin root directory
...
@pytest_asyncio.fixture(scope="function")
async def two_wallet_nodes_custom_spam_filtering(spam_filter_after_n_txs, llc_spam_amount):
    async for _ in setup_simulators_and_wallets(1, 2, {}, spam_filter_after_n_txs, llc_spam_amount):
        yield _
...
@pytest.fixture(scope="function")
def tmp_littlelambocoin_root(tmp_path):
    """
    Create a temp directory and populate it with an empty littlelambocoin_root directory.
    """
    path: Path = tmp_path / "littlelambocoin_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_littlelambocoin_root) -> Path:
    """
    Create a temp littlelambocoin_root directory and populate it with a default config.yaml.
    Returns the littlelambocoin_root path.
    """
    root_path: Path = tmp_littlelambocoin_root
