...
@pytest.mark.asyncio
async def test_help(littlelambocoin_root: LittlelambocoinRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = littlelambocoin_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.xfail(strict=True)
@pytest.mark.asyncio
def test_round_trip(littlelambocoin_root: LittlelambocoinRoot, littlelambocoin_daemon: None, littlelambocoin_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with littlelambocoin_root.print_log_after():
        create = littlelambocoin_root.run(args=["data", "create_data_store"])
        ...
        update = littlelambocoin_root.run(
            args=["data", "update_data_store", "--id", tree_id, "--changelist", json.dumps(changelist)]
        )
        dic = json.loads(create.stdout)
        assert dic["success"]
        print(f"update_data_store: {update}")
        completed_process = littlelambocoin_root.run(args=["data", "get_value", "--id", tree_id, "--key", key])
        parsed = json.loads(completed_process.stdout)
        expected = {"value": value, "success": True}
        assert parsed == expected
        get_keys_values = littlelambocoin_root.run(args=["data", "get_keys_values", "--id", tree_id])
