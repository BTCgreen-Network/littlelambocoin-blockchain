...
from littlelambocoin.data_layer.data_layer_util import NodeType, Side, Status
...
class LittlelambocoinRoot:
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        kwargs["env"]["LITTLELAMBOCOIN_ROOT"] = os.fspath(self.path)
        kwargs["env"]["LITTLELAMBOCOIN_KEYS_ROOT"] = os.fspath(self.path)
        ...
        modified_args: List[Union[str, os_PathLike_str]] = [
            self.scripts_path.joinpath("littlelambocoin"),
            "--root-path",
            self.path,
            *args,
        ]
