...
from littlelambocoin.data_layer.data_layer_util import NodeType, Status
from littlelambocoin.data_layer.data_store import DataStore
from littlelambocoin.types.blockchain_format.tree_hash import bytes32
...
def littlelambocoin_root_fixture(tmp_path: pathlib.Path, scripts_path: pathlib.Path) -> LittlelambocoinRoot:
...
def closing_littlelambocoin_root_popen(littlelambocoin_root: LittlelambocoinRoot, args: List[str]) -> Iterator[None]:
    environment = {**os.environ, "LITTLELAMBOCOIN_ROOT": os.fspath(littlelambocoin_root.path)}
...
@pytest.fixture(name="littlelambocoin_daemon", scope="function")
def littlelambocoin_daemon_fixture(littlelambocoin_root: LittlelambocoinRoot) -> Iterator[None]:
    with closing_littlelambocoin_root_popen(littlelambocoin_root=littlelambocoin_root, args=[sys.executable, "-m", "littlelambocoin.daemon.server"]):
...
@pytest.fixture(name="littlelambocoin_data", scope="function")
def littlelambocoin_data_fixture(littlelambocoin_root: LittlelambocoinRoot, littlelambocoin_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_littlelambocoin_root_popen(littlelambocoin_root=littlelambocoin_root, args=[os.fspath(scripts_path.joinpath("littlelambocoin_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
