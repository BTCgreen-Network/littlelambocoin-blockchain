...
from littlelambocoin.rpc.data_layer_rpc_client import DataLayerRpcClient
...
from littlelambocoin.util.config import lock_and_load_config, save_config
from littlelambocoin.util.ints import uint16
from littlelambocoin.util.misc import sendable_termination_signals
from tests.core.data_layer.util import LittlelambocoinRoot
from tests.util.misc import closing_littlelambocoin_root_popen
...
@pytest.mark.asyncio
async def test_daemon_terminates(signal_number: signal.Signals, littlelambocoin_root: LittlelambocoinRoot) -> None:
    port = find_available_listen_port()
    with lock_and_load_config(root_path=littlelambocoin_root.path, filename="config.yaml") as config:
        config["daemon_port"] = port
        save_config(root_path=littlelambocoin_root.path, filename="config.yaml", config_data=config)

    with closing_littlelambocoin_root_popen(littlelambocoin_root=littlelambocoin_root, args=[sys.executable, "-m", "littlelambocoin.daemon.server"]) as process:
        client = await wait_for_daemon_connection(root_path=littlelambocoin_root.path, config=config)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        [DataLayerRpcClient.create, "littlelambocoin.server.start_data_layer", "data_layer"],
        [FarmerRpcClient.create, "littlelambocoin.server.start_farmer", "farmer"],
        [FullNodeRpcClient.create, "littlelambocoin.server.start_full_node", "full_node"],
        [HarvesterRpcClient.create, "littlelambocoin.server.start_harvester", "harvester"],
        [WalletRpcClient.create, "littlelambocoin.server.start_wallet", "wallet"],
        # TODO: review and somehow test the other services too
        # [, "littlelambocoin.server.start_introducer", "introducer"],
        # [, "littlelambocoin.seeder.start_crawler", ""],
        # [, "littlelambocoin.server.start_timelord", "timelord"],
        # [, "littlelambocoin.timelord.timelord_launcher", ],
        # [, "littlelambocoin.simulator.start_simulator", ],
        # [, "littlelambocoin.data_layer.data_layer_server", "data_layer"],
    ],
)
@pytest.mark.asyncio
async def test_services_terminate(
    signal_number: signal.Signals,
    littlelambocoin_root: LittlelambocoinRoot,
    ...
    with closing_littlelambocoin_root_popen(
        littlelambocoin_root=littlelambocoin_root,
        args=[sys.executable, "-m", "littlelambocoin.daemon.server"],
    ):
        ...
        # farmer.
        daemon_client = await wait_for_daemon_connection(root_path=littlelambocoin_root.path, config=config)
        ...
        with closing_littlelambocoin_root_popen(
            ...
            client = await create_service(
                ...
                root_path=littlelambocoin_root.path,
                net_config=config,
            )
            try:
                start = time.monotonic()
