...
from littlelambocoin.rpc.farmer_rpc_api import (
    FilterItem,
...
from littlelambocoin.simulator.block_tools import get_plot_dir
from littlelambocoin.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
from littlelambocoin.util.config import load_config, lock_and_load_config, save_config
...
async def test_farmer_signage_point_endpoints(harvester_farmer_environment):
    ...
    sp = farmer_protocol.NewSignagePoint(
        std_hash(b"1"), std_hash(b"2"), std_hash(b"3"), uint64(1), uint64(1000000), uint8(2), ConsensusConstants.TIMELORD_PUZZLE_HASH,
    )
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "llc"), encode_puzzle_hash(new_ph_2, "llc"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "llc"), encode_puzzle_hash(observer_pool, "llc")
    )
    ...
    assert config["farmer"]["llc_target_address"] == encode_puzzle_hash(observer_farmer, "llc")
    assert config["pool"]["llc_target_address"] == encode_puzzle_hash(observer_pool, "llc")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "llc")
