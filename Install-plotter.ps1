...
function get_madmax_filename()
{
    ...
    $littlelambocoin_plot = "littlelambocoin_plot"
    if ("${ksize}" -eq "k34")
    {
        $littlelambocoin_plot = "littlelambocoin_plot_k34"
    }
    $suffix = ""
    if ("${os}" -eq "macos")
    {
    ...
    "${littlelambocoin_plot}-${ver}${suffix}"
}
...
{
    Write-Output "This script requires that the Littlelambocoin Python virtual environment is activated."
...
try {
    ...
    elseif("${plotter}" -eq "madmax")
    {
        ...
        Write-Output "Fetching binary from: ${URL}"
        try {
            Invoke-WebRequest -Uri "$URL" -Outfile "littlelambocoin_plot.exe"
            Write-Output "Successfully downloaded: $URL"
            Write-Output "Successfully installed madmax to $(Get-Location)\littlelambocoin_plot.exe"
        }
        catch {
            Write-Output "ERROR: Download failed. Maybe specified version of the binary does not exist."
            Pop-Location
        ...
        Write-Output "Fetching binary from: ${URL}"
        try {
            Invoke-WebRequest -Uri "$URL" -Outfile "littlelambocoin_plot_k34.exe"
            Write-Output "Successfully downloaded: $URL"
            Write-Output "Successfully installed madmax for k34 to $(Get-Location)\littlelambocoin_plot_k34.exe"
        }
        catch {
