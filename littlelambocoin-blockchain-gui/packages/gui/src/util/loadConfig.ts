...
export function getConfigRootDir(net = 'mainnet'): string {
  ...
  return 'LITTLELAMBOCOIN_ROOT' in process.env ? untildify(process.env.LITTLELAMBOCOIN_ROOT) : path.join(homedir, '.littlelambocoin', net);
}
...
export default async function loadConfig(net?: string): Promise<{
  ...
  try {
    ...
    const daemonPort = get(config, 'ui.daemon_port', 44453);
