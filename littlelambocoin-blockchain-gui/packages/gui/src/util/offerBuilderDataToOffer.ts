...
import { littlelambocoinToMojo, catToMojo } from '@littlelambocoin/core';
...
export default async function offerBuilderDataToOffer(
  ...
  const {
    offered: { llc: offeredXch = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { llc: requestedXch = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? littlelambocoinToMojo(firstFee.amount) : new BigNumber(0);
  ...
  await Promise.all(
    offeredXch.map(async (llc) => {
      const { amount } = llc;
      if (!amount || amount === '0') {
        throw new Error(t`Please enter an LLC amount`);
      }
  ...
  requestedXch.forEach((llc) => {
    ...
    walletIdsAndAmounts[wallet.id] = littlelambocoinToMojo(amount);
  });
