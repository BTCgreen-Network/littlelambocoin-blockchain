...
export function getUserDataDir(): string {
  const littlelambocoinRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(littlelambocoinRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const littlelambocoinRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${littlelambocoinRootUserDataPath}`);
  app.setPath('userData', littlelambocoinRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(littlelambocoinRootUserDataPath, 'Local Storage', 'leveldb');
