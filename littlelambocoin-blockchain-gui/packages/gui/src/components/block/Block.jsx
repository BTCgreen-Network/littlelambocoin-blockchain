...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@littlelambocoin/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToLittlelambocoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToLittlelambocoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const littlelambocoinFees = blockRecord.fees !== undefined
    ? mojoToLittlelambocoin(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.littlelambocoinexplorer.com/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.littlelambocoinexplorer.com/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
