import { Wallet, WalletType, type CATToken } from '@littlelambocoin/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@littlelambocoin/api-react';
import { Select, useCurrencyCode } from '@littlelambocoin/core';
...
type BuildAssetSelectorListParams = {
  ...
  littlelambocoinCurrencyCode: string;
};

function buildAssetSelectorList(params: BuildAssetSelectorListParams): WalletOfferAssetSelection[] {
  const { wallets, catList, rows, otherRows, selectedWalletId, littlelambocoinCurrencyCode } = params;
  ...
  wallets.map((wallet) => {
    ...
    if (wallet.type === WalletType.STANDARD_WALLET) {
      ...
      symbol = littlelambocoinCurrencyCode;
...
function OfferAssetSelector(props: OfferAssetSelectorProps) {
  ...
  const options: WalletOfferAssetSelection[] = useMemo(() => {
    ...
    return buildAssetSelectorList({
      ...
      otherRows,
      selectedWalletId,
      littlelambocoinCurrencyCode: currencyCode,
    });
