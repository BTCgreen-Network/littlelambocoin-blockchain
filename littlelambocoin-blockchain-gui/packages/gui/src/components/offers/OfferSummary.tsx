...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToLittlelambocoin, mojoToCAT } from '@littlelambocoin/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['llc', 'tllc'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToLittlelambocoin(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['llc', 'tllc'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToLittlelambocoin(takerEntries[0][1])
