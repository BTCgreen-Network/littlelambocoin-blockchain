...
import type { OfferSummaryAssetInfo, OfferSummaryInfos, OfferSummaryRecord } from '@littlelambocoin/api';
import { mojoToCAT, mojoToLittlelambocoin, mojoToCATLocaleString, mojoToLittlelambocoinLocaleString } from '@littlelambocoin/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> LLC assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToLittlelambocoinLocaleString(amount, locale);
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.LITTLELAMBOCOIN]) {
    ...
    if (assetId) {
      ...
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.LITTLELAMBOCOIN ? mojoToLittlelambocoin(amountInMojos) : mojoToCAT(amountInMojos);
