...
import { useCheckOfferValidityMutation, useGetNFTInfoQuery, useGetNFTWallets } from '@littlelambocoin/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['llc', 'tllc'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.LITTLELAMBOCOIN: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const llcMakerFee = mojoToLittlelambocoin(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(llcMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.LITTLELAMBOCOIN
        ? littlelambocoinToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
