...
const log = debug('littlelambocoin-gui:offers');
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. LLC/RandoCoin
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<string> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'tllc' : 'llc'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'tllc' : 'llc'}/offer/${id}`;
}
...
const KeybaseTeamName = 'littlelambocoin_offers';
