import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@littlelambocoin/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@littlelambocoin/api';
import { useCalculateRoyaltiesForNFTsQuery, useGetNFTsByNFTIDsQuery, useGetWalletsQuery } from '@littlelambocoin/api-react';
import { catToMojo, littlelambocoinToMojo } from '@littlelambocoin/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedLLC = useWatch({
    name: 'requested.llc',
  });

  const offeredLLC = useWatch({
    name: 'offered.llc',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedLLC ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('llc', littlelambocoinToMojo(amount))),
    ...(requestedTokens ?? [])
