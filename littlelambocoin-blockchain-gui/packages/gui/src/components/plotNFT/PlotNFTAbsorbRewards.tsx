import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@littlelambocoin/api-react';
import {
  UnitFormat,
  CardStep,
  ButtonLoading,
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const { fee } = data;
      const feeMojos = littlelambocoinToMojo(fee);
        ...
        <Flex flexDirection="column" gap={3}>
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat value={mojoToLittlelambocoinLocaleString(balance)} display="inline" state={State.SUCCESS} /> to{' '}
                {address}
              </Trans>
            </Typography>
