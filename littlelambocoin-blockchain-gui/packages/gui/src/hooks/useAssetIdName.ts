...
import { CATToken, Wallet, useCurrencyCode } from '@littlelambocoin/core';
...
export default function useAssetIdName() {
  ...
  const { assetIdNameMapping, walletIdNameMapping } = useMemo(() => {
    ...
    // If using testnet, add a TLLC assetId entry
    if (currencyCode === 'TLLC') {
      const assetId = 'tllc';
      const name = 'Littlelambocoin (Testnet)';
      const symbol = 'TLLC';
      const displayName = symbol || name;
      const entry: AssetIdMapEntry = {
        walletId: 1,
