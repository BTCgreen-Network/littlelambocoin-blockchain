...
import { TransactionType } from '@littlelambocoin/api';
import type { Transaction } from '@littlelambocoin/api';
import { mojoToLittlelambocoin, blockHeightToTimestamp } from '@littlelambocoin/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToLittlelambocoin(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToLittlelambocoin(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToLittlelambocoin(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToLittlelambocoin(start).toString, // bignumber is not supported by react
    });
  });
