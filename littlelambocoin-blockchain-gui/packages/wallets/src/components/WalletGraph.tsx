import { TransactionType, WalletType } from '@littlelambocoin/api';
import type { Transaction } from '@littlelambocoin/api';
import { useGetWalletBalanceQuery } from '@littlelambocoin/api-react';
import { mojoToLittlelambocoin, mojoToCAT, blockHeightToTimestamp } from '@littlelambocoin/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToLittlelambocoin(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToLittlelambocoin(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToLittlelambocoin(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToLittlelambocoin(start).toString(), // bignumber is not supported by react
    });
  });
