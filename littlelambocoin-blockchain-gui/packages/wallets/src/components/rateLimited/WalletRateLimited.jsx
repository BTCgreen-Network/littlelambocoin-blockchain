import { AlertDialog, Card, Flex, littlelambocoinToMojo, mojoToLittlelambocoinLocaleString } from '@littlelambocoin/core';
...
function IncompleteCard(props) {
  ...
  function submit() {
    ...
    const hellceck = /[\da-f]+$/gi;

    if (!hellceck.test(ip_val) || ip_val.value === '') {
    ...
    const littlelambocoinper_input = ip_parsed.limit;
    ...
    const littlelambocoinper_value = Number.parseInt(Number(littlelambocoinper_input));
    const origin_parsed = JSON.parse(origin_input);
    dispatch(rl_set_user_info_action(id, interval_value, littlelambocoinper_value, origin_parsed, admin_pubkey_input));
  }
      ...
      <Card title={<Trans>Rate Limited Info</Trans>}>
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>Spending Limit (littlelambocoin per interval): {mojoToLittlelambocoinLocaleString(limit)}</Trans>
              </Typography>
            </Box>
          </Box>
        </Grid>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToLittlelambocoinLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props) {
  ...
  function send() {
    ...
    const amount = littlelambocoinToMojo(amount_input.value);
    const fee = littlelambocoinToMojo(fee_input.value);
