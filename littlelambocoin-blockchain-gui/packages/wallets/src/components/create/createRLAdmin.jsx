import { AlertDialog, littlelambocoinToMojo } from '@littlelambocoin/core';
...
export function CreateRLAdminWallet() {
  ...
  let littlelambocoinper_input = null;
  ...
  function create() {
    ...
    if (
      littlelambocoinper_input.value === '' ||
      Number(littlelambocoinper_input.value) === 0 ||
      !Number(littlelambocoinper_input.value) ||
      isNaN(Number(littlelambocoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const littlelambocoinper = littlelambocoinToMojo(littlelambocoinper_input.value);
    const littlelambocoinper_value = Number.parseInt(Number(littlelambocoinper));
    ...
    // var fee = littlelambocoinToMojo(fee_input.value);
    // TODO(lipa): send fee to server
    // const fee_value = parseInt(Number(fee));
    dispatch(create_rl_admin_action(interval_value, littlelambocoinper_value, userpubkey, amount_value));
  }
        ...
        <Box display="flex">
          ...
          <Box flexGrow={6}>
            <TextField
              ...
              fullWidth
              inputRef={(input) => {
                littlelambocoinper_input = input;
              }}
