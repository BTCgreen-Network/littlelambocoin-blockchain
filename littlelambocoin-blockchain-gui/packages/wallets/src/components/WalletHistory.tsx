...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToLittlelambocoinLocaleString, mojoToCATLocaleString } from '@littlelambocoin/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@littlelambocoin/api-react';
import styled from 'styled-components';
import type { Row } from '@littlelambocoin/core';
import { WalletType, TransactionType } from '@littlelambocoin/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToLittlelambocoinLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToLittlelambocoinLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
