...
import { useGetWalletsQuery, useGetStrayCatsQuery, useGetCatListQuery, useAddCATTokenMutation } from '@littlelambocoin/api-react';
import { useShowError } from '@littlelambocoin/core';
  ...
  const list = useMemo(() => {
    ...
    let tokens = [
      ...baseWallets.map((wallet: Wallet) => ({
        ...
        name: wallet.type === WalletType.STANDARD_WALLET ? 'Littlelambocoin' : wallet.meta?.name ?? wallet.name,
      })),
