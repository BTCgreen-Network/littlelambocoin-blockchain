...
export default function SettingsApp(props: SettingsAppProps) {
  ...
  async function handleOpenFAQURL(): Promise<void> {
    try {
      ...
      await shell.openExternal('https://github.com/BTCgreen-Network/littlelambocoin-blockchain/wiki/FAQ');
    } catch (error: any) {
      showError(error);
    }
  }
  ...
  async function handleOpenSendFeedbackURL(): Promise<void> {
    try {
      const shell: Shell = (window as any).shell;
      await shell.openExternal('https://feedback.littlelambocoinnetwork.org/lightwallet');
    } catch (error: any) {
      showError(error);
    }
  }
