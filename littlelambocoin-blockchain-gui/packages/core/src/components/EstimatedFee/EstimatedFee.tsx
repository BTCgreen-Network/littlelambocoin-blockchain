import { useGetFeeEstimateQuery } from '@littlelambocoin/api-react';
import { Fee, Flex, mojoToLittlelambocoinLocaleString, useCurrencyCode, useLocale } from '@littlelambocoin/core';
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const txCostEstimates = {
    walletSendLLC: Math.floor(maxBlockCostCLVM / 1170),
    spendCATtx: 36382111,
  ...
  function formatEst(number, multiplier, locale) {
    ...
    const formatNum = mojoToLittlelambocoinLocaleString(num, locale);
    return formatNum;
  }
