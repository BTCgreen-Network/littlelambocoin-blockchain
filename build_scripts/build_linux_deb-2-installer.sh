...
export LITTLELAMBOCOIN_INSTALLER_VERSION
...
pip install j2cli
CLI_DEB_BASE="littlelambocoin-blockchain-cli_$LITTLELAMBOCOIN_INSTALLER_VERSION-1_$PLATFORM"
mkdir -p "dist/$CLI_DEB_BASE/opt/littlelambocoin"
mkdir -p "dist/$CLI_DEB_BASE/usr/bin"
mkdir -p "dist/$CLI_DEB_BASE/DEBIAN"
j2 -o "dist/$CLI_DEB_BASE/DEBIAN/control" assets/deb/control.j2
cp -r dist/daemon/* "dist/$CLI_DEB_BASE/opt/littlelambocoin/"
ln -s ../../opt/littlelambocoin/littlelambocoin "dist/$CLI_DEB_BASE/usr/bin/littlelambocoin"
...
cd ../littlelambocoin-blockchain-gui/packages/gui || exit 1
...
if [ "$PLATFORM" = "arm64" ]; then
  ...
  echo USE_SYSTEM_FPM=true electron-builder build --linux deb --arm64 \
    --config.productName="$PRODUCT_NAME" --config.linux.desktop.Name="Littlelambocoin Blockchain" \
    --config.deb.packageName="littlelambocoin-blockchain"
...
GUI_DEB_NAME=littlelambocoin-blockchain_${LITTLELAMBOCOIN_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${LITTLELAMBOCOIN_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"
