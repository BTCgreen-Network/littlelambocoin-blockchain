...
cd ./littlelambocoin-blockchain-gui || exit 1
...
# Other than `littlelambocoin-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, here unused packages are removed.
echo "Remove unused @littlelambocoin packages to make cache slim"
ls -l packages
rm -rf packages/api
...
# Remove `packages/gui/node_modules/@littlelambocoin` because it causes an error on later `electron-packager` command
rm -rf "@littlelambocoin"
